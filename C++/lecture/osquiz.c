...
void DiningPhil_PickUp(DiningPhil *dp, int id)
{
    ...
    DiningPhil_Test(dp, id, id);
    while (dp->state[id] != EATING)
    {
        pthread_cond_wait(&dp->self[id], &dp->mutex);
    }
...
void DiningPhil_PutDown(DiningPhil *dp, int id)
{
    ...
    //		- do not make it mutually exclusive unless it is necessary
    int leftPhil = (id + 4) % 5;
    int rightPhil = (id + 1) % 5;

    pthread_mutex_lock(&dp->mutex);
    dp->state[id] = THINKING;

    DiningPhil_Test(dp, leftPhil, id);
    DiningPhil_Test(dp, rightPhil, id);

    pthread_mutex_unlock(&dp->mutex);

    pthread_cond_signal(&dp->self[leftPhil]);
    pthread_cond_signal(&dp->self[rightPhil]);
...
// e.g., Philosoper 2 can call this function as 'DiningPhil_Test(dp, 2, 2);' to test her/himself.
{
    ...
    if (dp->state[leftPhil] != EATING && dp->state[id] == HUNGRY && dp->state[rightPhil] != EATING)
    {
        dp->state[id] = EATING;
        pthread_cond_signal(&dp->self[id]);
    }
...
// end of file <problem for section 01>
